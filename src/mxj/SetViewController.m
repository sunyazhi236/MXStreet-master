...
{
    NSString *messagePushFlag; //消息推送开关标记
}
...
- (void)viewDidLoad {
    ...
    messagePushFlag = [LoginModel shareInstance].pushMessage;
    //调整推送按钮的状态
    if ([messagePushFlag isEqualToString:@"1"]) {
        [self.messagePushBtn setBackgroundImage:[UIImage imageNamed:@"on_12"] forState:UIControlStateNormal];
    } else if([messagePushFlag isEqualToString:@"0"]) {
...
-(void)messagePushBtnClick
{
    ...
    [ModifyUserDataInput shareInstance].pushMessage = messagePushFlag;
    ...
    [[NetInterface shareInstance] modifyUserData:@"modifyUserData" param:dict successBlock:^(NSDictionary *responseDict) {
        ...
        if (RETURN_SUCCESS(returnData.status)) {
            [LoginModel shareInstance].pushMessage = messagePushFlag;
        } else {
